...
const ListingDetailScreen = ({ route, navigation }) => {
  console.log("item id---->",route.params.listingId)
  ...
  useEffect(() => {
    ...
    api.get(`listings/${route.params.listingId}`).then((res) => {
      console.log(res?.data.contact.whatsapp_number)
      if (res.ok) {
  ...
  const handleWhatsapp = () => {
    Linking.openURL(
        ...
        listingData.contact.whatsapp_number==null ?listingData.contact.phone:listingData.contact.whatsapp_number
    )
  }
  return (
      <View style={{flex: 1}}>
        <ListingHeader
            title={("listingDetailScreenTexts.pageTitle", appSettings.lng)}
                              ...
                              {Object.keys(listingData.social_profiles).map(
                                  (_profile, index) => (
                                      <TouchableOpacity
                                          ...
                                          key={_profile}
                                          onPress={() => handleSocialProfileClick(_profile)}
                                      >
                                        <FontAwesome
                                            name={_profile}
                                            size={20}
